...
import { User, Save, Eye, EyeOff, Phone } from 'lucide-react'
...
export default function ProfilePage() {
  ...
  const handleSave = async () => {
    setSaving(true)
    setMessage('')
    const body: Record<string, any> = { name, email, phone }
    if (newPassword) {
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            <div><label style={labelStyle}>Email</label>
              <input value={email} onChange={(e) => setEmail(e.target.value)} style={inputStyle}
                onFocus={e => e.target.style.borderColor = '#6366f1'} onBlur={e => e.target.style.borderColor = 'rgba(42,42,58,0.8)'} /></div>
            <div><label style={{ ...labelStyle, display: 'flex', alignItems: 'center', gap: '6px' }}><Phone size={13} /> Phone</label>
            ...
            {message && (
              <p style={{ color: message === 'Profile updated successfully' ? '#22d3a0' : '#f43f5e', fontSize: '14px', fontWeight: '500', textAlign: 'center' }}>{message}</p>
            )}
